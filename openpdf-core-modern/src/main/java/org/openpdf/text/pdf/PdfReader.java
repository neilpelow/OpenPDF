...
public class PdfReader implements PdfViewerPreferences, Closeable {
    ...
    private void readDecryptedDocObj() throws IOException {
        ...
        if (documentIDs != null) {
            ...
            strings.remove(o);
            s = o.toString();
            documentID = org.openpdf.text.DocWriter.getISOBytes(s);
        ...
        if (filter.equals(PdfName.STANDARD)) {
            ...
            uValue = org.openpdf.text.DocWriter.getISOBytes(s);
            s = enc.get(PdfName.O).toString();
            strings.remove(enc.get(PdfName.O));
            oValue = org.openpdf.text.DocWriter.getISOBytes(s);
        ...
        if (filter.equals(PdfName.STANDARD)) {
            if (rValue < 6) {
                ...
                byte[] ueValue = org.openpdf.text.DocWriter.getISOBytes(s);
                s = enc.get(PdfName.OE).toString();
                strings.remove(enc.get(PdfName.OE));
                byte[] oeValue = org.openpdf.text.DocWriter.getISOBytes(s);
                s = enc.get(PdfName.PERMS).toString();
                strings.remove(enc.get(PdfName.PERMS));
                byte[] permsValue = org.openpdf.text.DocWriter.getISOBytes(s);
    ...
    public byte[] getDocumentId() {
        ...
        return org.openpdf.text.DocWriter.getISOBytes(o.toString());
    }
